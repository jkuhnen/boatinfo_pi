# ---------------------------------------------------------------------------
# Author:      Jon Gough based on the work of Pavel Kalian (Based on the work of
# Sean D'Epagnier) Copyright:   2020 License:     GPLv3+
# ---------------------------------------------------------------------------
...
if (NOT DEFINED GIT_REPOSITORY_SERVER)
  ...
  message(
    STATUS
      "${CMLOC}GIT_REPOSITORY_SERVER not found setting to: ${GIT_REPOSITORY_SERVER}"
  )
endif ()
...
message(
  STATUS
    "${CMLOC}${VERBOSE_NAME} Version: ${VERSION_MAJOR}.${VERSION_MINOR}.${VERSION_PATCH}.${VERSION_TWEAK}"
)
...
elseif (MSVC)
  ...
  message(
    STATUS
      "${CMLOC}PKG_TARGET_WX_VER: ${PKG_TARGET_WX_VER}, WX_VER: $ENV{WX_VER}"
  )
...
elseif (APPLE)
  ...
  if (NOT "$ENV{WX_VER}" STREQUAL "")
    set(PKG_TARGET "darwin-wx$ENV{WX_VER}")
  else ()
...
elseif (UNIX)
  # Some linux dist:
  execute_process(
    COMMAND "lsb_release" "-rs" OUTPUT_VARIABLE PKG_TARGET_VERSION
  )
  if ("$ENV{BUILD_ENV}" STREQUAL "debian" OR "$ENV{BUILD_ENV}" STREQUAL
                                             "DEBIAN"
  )
    set(PKG_TARGET "$ENV{BUILD_ENV}")
    if ("${PKG_TARGET_VERSION}" STRLESS "12")
      if (NOT "$ENV{WX_VER}" STREQUAL "")
        set(PKG_TARGET_WX_VER "-wx$ENV{WX_VER}")
      endif ()
    endif ()
  ...
  elseif (
    ("$ENV{BUILD_ENV}" STREQUAL "ubuntu" OR "$ENV{BUILD_ENV}" STREQUAL "UBUNTU")
    AND (("$ENV{OCPN_TARGET}" STREQUAL "jammy" OR "$ENV{OCPN_TARGET}" STREQUAL
                                                  "JAMMY"))
  )
...
if (NOT WIN32 AND NOT QT_ANDROID)
  ...
  if (UNIX AND NOT APPLE)
    ...
    if (EXISTS /etc/debian_version)
      ...
      set(PACKAGE_DEPS
          "libc6, libwxgtk3.0-0, wx-i18n, libglu1-mesa (>= 7.0.0), libgl1-mesa-glx (>= 7.0.0), zlib1g, bzip2, libportaudio2"
      )
      ...
      if (CMAKE_SYSTEM_PROCESSOR MATCHES "arm*")
        if (CMAKE_SIZEOF_VOID_P MATCHES "8")
          if ("$ENV{OCPN_TARGET}" MATCHES "flatpak")
            ...
            if ("$ENV{BUILD_ARCH}" MATCHES "aarch64")
              set(ARCH "aarch64")
            else ("$ENV{BUILD_ARCH}" MATCHES "aarch64")
              set (ARCH "x86_64")
            endif ("$ENV{BUILD_ARCH}" MATCHES "aarch64")
...
message(
  STATUS
    "${CMLOC}PKG_TARGET: ${PKG_TARGET}, PKG_TARGET_VERSION: ${PKG_TARGET_VERSION}"
)
...
if (Plugin_CXX11)
  ...
  elseif (COMPILER_SUPPORTS_CXX0X)
    ...
    message(
      STATUS
        "${CMLOC}The compiler ${CMAKE_CXX_COMPILER} has no C++11 support. Please use a different C++ compiler."
    )
